...
ServerEvents.recipes((x) => {
    ...
    for (const metal of Object.keys(metals)) {
        ...
        console.log(
          `Adding recipes for ${crushed}`,
          `Blasting: ${crushed} -> ${nugget}`,
          `Seething: ${crushed} -> ${nugget} + .75% ${nugget}`
        );
        ...
        if (compressed) {
            ...
            console.log(
              `Adding recipes for ${compressedBlock}`,
              `Blasting: ${compressedBlock} -> 9x ${nugget}`,
              `Seething: ${compressedBlock} -> 9x ${nugget} + 7x .75% ${nugget}`
            );
